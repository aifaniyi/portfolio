          ...
          return (
            <div key={project.id}>
              ...
              <TimelineItem position={position}>
                <TimelineDot
                  className={`top-4 ${getCategoryColor(project.category)} ring-border`}
                />

                {/* Date - hidden on mobile (shown inside card instead) */}
                <div
                  className={`hidden md:block absolute top-3 ${position === 'right' ? 'left-[calc(50%+2rem)]' : 'right-[calc(50%+2rem)]'}`}
                >
                  {/* Date sits on the opposite side of the card */}
                </div>

                <TimelineContent
                  className={position === 'left' ? 'md:ml-auto' : ''}
                >
                  <div
